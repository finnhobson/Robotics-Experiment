...
int Mapper::returnEmptySquare() {
  int loc = 0;
  bool found = false;
  int randomLoc = random(0, (MAP_RESOLUTION-1) * MAP_RESOLUTION + (MAP_RESOLUTION -1) );
  byte value;
  value = EEPROM.read(randomLoc);
  while ((char)value != '#'){
    randomLoc = random(0, (MAP_RESOLUTION-1) * MAP_RESOLUTION + (MAP_RESOLUTION -1) );
    value = EEPROM.read(randomLoc);
  }
  loc = randomLoc;
  return loc;
