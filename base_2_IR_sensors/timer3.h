...
extern volatile long left_count_prior;
extern volatile long right_count;
extern volatile long right_count_prior;
...
ISR( TIMER3_COMPA_vect ) {
  ...
  diff = left_count - left_count_prior;
  left_count_prior = left_count;
  
  // Save speed to global variable.
  l_speed_t3 = (float)diff;
  
  
  diff = right_count - right_count_prior;
  right_count_prior = right_count;
